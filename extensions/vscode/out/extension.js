    ...
    try {
        ...
        if (result.status === 'completed') {
            ...
            if (vulnerabilityCount > 0) {
                ...
                vscode.window.showWarningMessage(`GuardRail AI found ${vulnerabilityCount} security issue(s)`, 'View Issues', 'Apply Fixes').then(selection => {
        ...
        if (error.code === 'ECONNREFUSED') {
            vscode.window.showErrorMessage('GuardRail AI backend is not running. Please start the API server.', 'Open Settings').then(selection => {
